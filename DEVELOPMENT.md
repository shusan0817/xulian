...
> 本段記錄 `xulian` 專案從模板初始化到可交付的完整過程。
> 上方內容為 `codebuddy-chat-web` 模板原文，保留作參考。

## 開發時序

| 階段 | 內容 | 產出 |
|---|---|---|
| T00 | 環境準備 · 骨架初始化 | `xulian/` 專案目錄 |
| T01 | SDK/CLI 連線探測 | `scripts/probe-sdk.ts` |
| T02 | 資料庫 schema + Repository | 15 張表 |
| T03 | 8 層 Prompt 引擎 | `server/agent/prompts.ts` |
| T04–T09 | 情緒 / 用戶情緒 / 策略 / 記憶 / 關係 / 人格 | 6 個服務 |
| T10 | 主動聊天決策系統 | 11 否決 + 7 因子 |
| T11 | 主動訊息生成 + 調度器 | 含冪等鎖 |
| T12 | 推播通知 | `notificationService` + `sw.js` |
| T13 | 路由層 | 36 個端點 |
| T14 | 前端 | 6 頁 + 15 元件 + 5 hooks |
| T15 | 驗證與交付 | 本文件 + README |

---

## T01 · SDK / CLI 探測結論（重要）

這是整個專案的技術地基，結論直接決定了實作方向。
...
```
node_modules/@tencent-ai/agent-sdk/cli/bin/codebuddy     (19 MB, v2.143.1)
```

不需要 `npm install -g`，也不需要額外下載。

### 2. 兩種呼叫模式

| 模式 | 說明 |
|---|---|
| `path`（預設，推薦） | 直接 spawn CLI 執行檔 |
| `executableArgs` | Windows 上若 spawn 失敗的備案 |

用 `CODEBUDDY_CLI_MODE` 切換。本機 `path` 模式正常。
...
**這一句話很關鍵** —— 它證實呼叫真的到了 CodeBuddy 服務端，
不是本地 stub，也不是靜默失敗。

### 4. ⚠️ 最重要的結論：放棄 `unstable_v2_createSession()`

模板預設用 `unstable_v2_createSession()`，但它的 `SessionOptions`
**沒有 `systemPrompt` 欄位**。

「需恋」的人格、情緒、關係階段、應對策略**每一輪都在變**，
必須每輪重建 system prompt。
...
SDK 在不同情境下可能回傳「累積全文」或「增量片段」。
統一在伺服端做 diff，保證 SSE 永遠吐增量：
...
**現象**：連上 `/api/chat/stream` 後沒有任何事件，訊息也沒入庫。

**根因**：用了 `req.on('close')` 偵測客戶端斷線。
但 Node 16+ 中 `req` 的 `close` 是在 **body 讀完後**觸發，
不是客戶端斷線時。

**修復**：改成 `res.on('close')`。

**影響**：這是「能不能用」等級的 bug，修完後 SSE 事件序列完整。

---

### 🔴 2. `SqliteError: no such column: character_id`

**現象**：Scheduler 第一次 tick 直接崩潰。

**根因**：`ai_characters` 表的主鍵是 `id`，沒有 `character_id` 欄位。

**修復**：

```sql
SELECT user_id, id AS character_id FROM ai_characters
```

**後續**：為系統性防堵這類錯誤，寫了 `scripts/smoke-db.ts`，
涵蓋 76 項資料庫操作（含冪等鎖驗證）。

---

### 🟡 3. `simulateIdleHours` 方向錯誤（2026-09-05 修復）

**現象**：`/api/proactive/dry-run?simulateIdleHours=20` 永遠回傳
`V9_NO_CHANNEL` 或 `V11_NO_INTERACTION`，看不到真實打分過程。
...
往前撥會讓 `lastInteractionAt` 落在「未來」，
`minutesSinceLastChat` 變成**負數**，
被 V9/V11 誤判成「從未互動過」。
...
|---|---|---|
| 1h | 0.438 | skip |
| 6h | 0.510 | delay (113min) |
| 14h | 0.614 | delay (49min) |
| 25h | 0.669 | send |

---

### 🟡 4. 43 個 TypeScript 錯誤

主要幾類：

| 錯誤 | 修復 |
|---|---|
| 相對路徑層級錯（`../../` 應為 `../../../`） | `proactive/` 是 3 層深，sed 批次修正 |
| `proactivityLevel` 不存在於 `ProactiveSettings` | 改用 `character.proactivityLevel` |
| `Array<string\|number>` 型別不符 | 補 `\| null` |
| `sourceMessageId` 不在 `UpdateMemoryPatch` | 移除或改型別 |
| `adminFinishRunLock` 參數不足 | 補第 3 個參數 `'done'` |
| `iconMap.ts` lucide 型別 | 改用 `LucideIcon` |

**結果**：降至 **0 錯誤**。

---

### 🟡 5. better-sqlite3 原生二進位抓不到

**現象**：`npm install` 卡住 1h19m，卡在從 GitHub 下載原生檔。

**根因**：本環境 github.com 被擋。
...
或手動下載（ABI 127 / Node v22）：

```
https://registry.npmmirror.com/-/binary/better-sqlite3/v12.11.1/better-sqlite3-v12.11.1-node-v127-win32-x64.tar.gz
```

解壓到 `node_modules/better-sqlite3/build/Release/`。

---

### 🟢 6. 其他小修

- `web-push` 缺型別宣告 → `npm i -D @types/web-push`
- `MemoryPage` / `SettingsPage` 有重複的 `Switch` 宣告 → 抽到 `src/components/common/Switch.tsx`
- `proactiveSettings` 巢狀物件 PATCH 不會深合併 → 加 `patchProactive()` helper
- Service Worker 開發期會快取舊 HMR 模組 → 只在 `import.meta.env.PROD` 註冊
...
```
✅ npx tsc --noEmit              0 錯誤
✅ npx vite build                成功（472.84 kB / gzip 138.79 kB）
✅ npm run smoke                 76 通過 / 0 失敗
✅ npm run push:keys             VAPID 金鑰已生成並寫入 .env
✅ GET /api/health               {"status":"ok","cliAvailable":true,"database":true}
✅ GET /api/push/status          {"configured":true}
✅ proactive dry-run 曲線        skip → delay → delay → send
```

**規模**：92 個 TS/TSX 檔 · 15,125 行 · 15 張表 / 161 欄 · 36 個 API

---

## 待辦（下一階段）

1. 🔴 **使用者系統** —— 現在換個 `X-User-Id` 就能讀別人資料，上線前必須補
2. 🟡 **記憶向量檢索** —— 目前字面檢索，語義相近會漏
3. 🟡 **Scheduler 外部化** —— in-process `setInterval`，重啟重置、多實例重複發
4. 🟢 內容安全獨立審核
5. 🟢 多模態（語音/圖片）
