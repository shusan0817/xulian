...
到这一步，网站**新产生**的数据已经存到云端了。
但**之前**在网站上注册过的账号、聊过的记录，还在旧的临时盘里，需要跑一次搬家。
...
跑完会逐表打印搬运结果，最后出现 `✓ 迁移完成` 就成功了。

> 这个脚本是**安全**的：源数据库以**只读**方式打开，**绝不删除你本地的任何文件**，
> 而且可以反复执行（重复跑不会产生重复数据）。

> 如果你本地没有 Node.js 环境，可以跳过这步 —— 代价只是「老用户需要重新注册」，不影响新数据。
...
**现象**：`dbMode` 已经是 `turso`，但老用户反馈「我的账号不见了」。

**原因**：新库是空的，第 7 步没做。

**处理**：补跑 `npm run db:migrate-turso`。
