 ...
 *
 * ⚠️ 这里是「纯粹按元数据更新」，**刻意不做任何 +1 保底**：
 *    能走到这个函数，说明模型守住了格式、服务端解析成功，
 *    那么 change 是多少就是多少——哪怕是 0，那也是模型的真实判断（这轮不值得加分）。
 *    若在这里补 +1，就会出现「模型说 0、心动值却涨了 1」的自相矛盾。
 *    「模型没守格式 / 拿不到元数据」的 +1 保底在 useChat.ts 的 done 分支：
 *    服务端那种情况**不带** favorability 字段，前端因此回退到 applyAiReply。
 *    A / B 两种语义的完整分工见 useChat.ts done 分支的注释，改这里前务必先看。
 *
